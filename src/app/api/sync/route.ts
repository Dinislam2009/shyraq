...
export async function GET(request:NextRequest){
 ...
 if(request.nextUrl.searchParams.get("bootstrap")==="1"){
  ...
  const [{data:cards,error:cardError},{data:templates,error:templateError},{data:tags,error:tagError},{data:collections,error:collectionError},{data:collectionCards,error:collectionCardError},{data:media}]=await Promise.all([
   deckIds.length
   ...
   workspaceIds.length ? supabase.from("collection_cards").select("collection_id,card_id,created_at").in("collection_id",[]).limit(1) : Promise.resolve({data:[],error:null}),
  ...
  if(deckError||cardError||templateError||tagError||collectionError||collectionCardError)return NextResponse.json({error:deckError?.message||cardError?.message||templateError?.message||tagError?.message||collectionError?.message||"Bootstrap failed."},{status:500});
