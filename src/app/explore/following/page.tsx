...
export default async function FollowingPage(){
 ...
 const ids=(follows??[]).map((f:any)=>f.decks?.owner_id).filter(Boolean);
 const {data:profiles}=ids.length?await supabase.from("profiles").select("id,username,display_name").in("id",[...new Set(ids)]):{data:[]};
