...
export default async function FollowingPage(){
 ...
 const locale=await getRequestLocale();const {data:{user}}=await supabase.auth.getUser();if(!user)return <AppShell><div className="mx-auto max-w-5xl px-5 py-10">Sign in to manage followed decks.</div></AppShell>;
 ...
 const {data:profiles}=visibleCreatorIds.length?await supabase.from("public_profiles").select("id,username,display_name").in("id",[...new Set(visibleCreatorIds)]):{data:[]};
