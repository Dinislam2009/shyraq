...
type SearchParams={q?:string;type?:string;visibility?:string;workspace?:string;saved?:string;error?:string};
...
export default async function SearchPage({searchParams}:{searchParams:Promise<SearchParams>}){
 ...
 const visibleCards=(cards??[]).filter((card:any)=>!workspaceId||workspaceDeckIds.includes(String(card.deck_id)));
 const visibleTags=(tags??[]).filter((tag:any)=>!workspaceId||String(tag.workspace_id)===workspaceId);
 const visibleCollections=(collections??[]).filter((collection:any)=>!workspaceId||String(collection.workspace_id)===workspaceId);
 ...
 return <AppShell><div className="mx-auto max-w-6xl px-5 py-8 sm:px-8">
  ...
  {query&&user?<div className="mt-4 rounded-2xl border border-black/[0.06] bg-white p-4"><div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold">Save this search</p><span className="text-xs text-slate-400">Recent saved searches are kept here.</span></div><form action={saveSearch} className="mt-3 flex flex-col gap-2 sm:flex-row"><input name="name" required placeholder="Search name" className="h-10 min-w-0 flex-1 rounded-xl border border-slate-200 px-3 text-sm"/><input type="hidden" name="query" value={query}/><input type="hidden" name="filters" value={JSON.stringify({type,visibility,workspace:workspaceId})}/><button className="rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white">Save</button></form><div className="mt-4 flex flex-wrap gap-2">{(savedSearches??[]).map((search:any)=><div key={search.id} className="flex items-center gap-2 rounded-full bg-slate-50 px-3 py-1.5"><a href={"/search?q="+encodeURIComponent(search.query)} className="max-w-48 truncate text-xs font-medium">{search.name}</a><form action={deleteSearch.bind(null,search.id)}><button className="text-[10px] font-semibold text-red-600">Delete</button></form></div>)}</div></div>:null}
  {query?<div className="mt-8 grid gap-5 lg:grid-cols-2"><ResultSection title="Decks" items={visibleDecks.map((x:any)=>({key:x.id,title:x.name,meta:x.visibility,href:"/decks/"+x.id,body:x.description}))}/><ResultSection title="Cards" items={visibleCards.map((x:any)=>({key:x.id,title:x.content?.front||"Untitled card",meta:x.kind,href:"/decks/"+x.deck_id,body:x.content?.back||""}))}/><ResultSection title="Tags" items={visibleTags.map((x:any)=>({key:x.id,title:x.name,meta:"tag",href:"/search?q="+encodeURIComponent(x.name)}))}/><ResultSection title="Collections" items={visibleCollections.map((x:any)=>({key:x.id,title:x.name,meta:x.kind,href:"/collections/"+x.id}))}/><ResultSection title="Creators" items={(profiles??[]).map((x:any)=>({key:x.id,title:x.display_name||x.username||"Creator",meta:x.username?"@"+x.username:"creator",href:x.username?"/u/"+x.username:"/settings/profile",body:x.bio||""}))}/></div>:<div className="mt-8 rounded-2xl border border-dashed border-slate-300 bg-white p-12 text-center text-sm text-slate-500">Press Cmd/Ctrl + K from the top bar or enter a query above.</div>}
 </div></AppShell>;
}
