 ...
 return <AppShell><div className="mx-auto max-w-6xl px-5 py-8 sm:px-8">
  ...
  <div className="mt-6 rounded-3xl border border-black/[0.06] bg-white p-8">
   <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
    {avatarSrc?<Image src={avatarSrc} alt="" width={64} height={64} sizes="64px" className="h-16 w-16 rounded-2xl object-cover"/>:<div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-100 text-xl font-bold">{String(profile.display_name||profile.username||"S").slice(0,1).toUpperCase()}</div>}
   ...
   <p className="mt-5 text-xs text-slate-400">{profile.show_activity!==false?"Creator activity is public.":"Creator activity is limited."} · Creator since {formatDate(profile.created_at,locale)} · {copies} tracked copies</p>
  </div>

  <section className="mt-8"><h2 className="text-lg font-semibold">Public decks</h2><div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{decks.map((deck:any)=><Link href={"/explore/"+deck.id} key={deck.id} className="overflow-hidden rounded-2xl border border-black/[0.06] bg-white hover:shadow-sm">{deck.settings?.coverUrl&&String(deck.settings.coverUrl).startsWith("https://")&&String(deck.settings.coverUrl).includes("supabase.co")?<div className="relative h-28 w-full"><Image src={String(deck.settings.coverUrl)} alt="" fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover"/></div>:null}<div className="p-6"><h3 className="font-semibold">{deck.name}</h3><p className="mt-2 line-clamp-2 text-sm text-slate-500">{deck.description||"No description"}</p><div className="mt-5 flex flex-wrap gap-1">{[deck.settings?.category,deck.settings?.subject,deck.settings?.language,deck.settings?.difficulty].filter(Boolean).map((value:any)=><span key={String(value)} className="rounded-md bg-slate-100 px-2 py-1 text-[10px] text-slate-500">{String(value)}</span>)}</div><p className="mt-5 text-xs text-slate-400">{deck.cards?.[0]?.count??0} cards</p></div></Link>)}</div>{!decks.length?<p className="mt-4 text-sm text-slate-400">No public decks yet.</p>:null}</section>
  ...
  {profile.show_activity!==false?<section className="mt-10"><h2 className="text-lg font-semibold">Recent public activity</h2><div className="mt-4 space-y-2">{publicActivity.map((entry:any)=><div key={entry.id} className="rounded-xl border border-black/[0.06] bg-white p-4"><p className="text-sm font-medium">{String(entry.event_type).replaceAll("."," ")} · {deckMap.get(entry.entity_id)?.name||"Deck"}</p><p className="mt-1 text-xs text-slate-400">{formatDateTime(entry.created_at,locale)}</p></div>)}{!publicActivity.length?<p className="mt-3 text-sm text-slate-400">No recent public activity.</p>:null}</div></section>:null}
 </div></AppShell>;
}
