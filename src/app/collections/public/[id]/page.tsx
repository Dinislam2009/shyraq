...
export default async function PublicCollectionPage({params}:{params:Promise<{id:string}>}){
 ...
 const {data:collection}=await supabase.from("collections").select("id,name,description,kind,is_public,owner_id,collection_cards(card_id)").eq("id",id).eq("is_public",true).maybeSingle();
