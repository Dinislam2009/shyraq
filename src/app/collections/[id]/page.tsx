...
export default async function CollectionPage({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<{sort?:string}>}){
 ...
 const {data:collection}=await supabase.from("collections").select("id,name,kind,owner_id,sort_mode,rule,is_public,collection_cards(card_id)").eq("id",id).eq("owner_id",user.id).maybeSingle();
