...
export default async function CollectionPage({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<{sort?:string}>}){
 ...
 const {data:collection}=await supabase.from("collections").select("id,name,kind,owner_id,workspace_id,sort_mode,rule,is_public,collection_cards(card_id)").eq("id",id).maybeSingle();
 if(!collection)notFound();
 const permission=await getEffectiveCollectionRole(id);
 const canEdit=permission.collectionRole==="editor";
 ...
 return <AppShell><div className="mx-auto max-w-6xl px-5 py-8 sm:px-8">
  <Link href="/collections" className="text-sm text-slate-400">← Collections</Link>{permission.workspaceRole==="owner"||permission.workspaceRole==="admin"?<Link href={"/collections/"+id+"/permissions"} className="ml-4 text-sm font-semibold text-slate-600">Permissions</Link>{collection.is_public?<Link href={"/collections/public/"+id} className="ml-4 text-sm font-semibold text-slate-600">Public view</Link>:null}:null}
  <div className="mt-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">{collection.kind}</p><h1 className="mt-2 text-3xl font-semibold">{collection.name}</h1><p className="mt-2 text-sm text-slate-500">{cards.length} cards · {collection.is_public?"Public":"Private"}</p></div>{canEdit&&collection.kind!=="favorites"?<form action={deleteCollection.bind(null,id)}><button className="rounded-xl border border-red-200 bg-white px-4 py-2.5 text-sm font-semibold text-red-600">Delete collection</button></form>:null}</div>
  <div className="mt-6 grid gap-4 md:grid-cols-[1fr_300px]">
   <div className="overflow-hidden rounded-2xl border border-black/[0.06] bg-white">
    {cards.map((c:any,i:number)=><div key={c.id} className={"flex items-start gap-4 p-5 "+(i?"border-t border-black/[0.05]":"")}><Link href={"/decks/"+c.deck_id} className="min-w-0 flex-1 hover:text-slate-600"><p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400">{c.kind}</p><p className="mt-2 truncate font-medium">{c.content?.front||"Untitled card"}</p><p className="mt-1 line-clamp-2 text-sm text-slate-500">{c.content?.back||""}</p></Link>{canEdit&&collection.kind!=="smart"?<form action={removeCardFromCollection.bind(null,id,c.id)}><button className="text-xs font-semibold text-slate-500 hover:text-red-600">Remove</button></form>:null}</div>)}
    {!cards.length?<div className="p-10 text-center text-sm text-slate-500">No cards match this collection.</div>:null}
   </div>
   <div className="space-y-4">
    {canEdit&&collection.kind!=="favorites"?<form action={updateCollection.bind(null,id)} className="rounded-2xl border border-black/[0.06] bg-white p-5"><h2 className="text-sm font-semibold">{collection.kind==="smart"?"Edit smart collection":"Edit collection"}</h2><label className="mt-3 flex items-center gap-2 text-sm"><input type="checkbox" name="is_public" defaultChecked={Boolean(collection.is_public)} className="h-4 w-4"/><span>Make this collection public</span></label><input name="name" defaultValue={collection.name} className="mt-3 h-10 w-full rounded-xl border border-slate-200 px-3 text-sm"/>{collection.kind==="smart"?<div className="mt-3 grid gap-2"><select name="rule_kind" className="h-10 rounded-xl border border-slate-200 px-2 text-sm"><option value="kind">Card kind</option><option value="marked">Marked</option><option value="suspended">Suspended</option><option value="tag">Tag</option></select><input name="rule_value" defaultValue={String(collection.rule?.tag||collection.rule?.kind||"")} placeholder="kind or tag name" className="h-10 rounded-xl border border-slate-200 px-3 text-sm"/></div>:null}<button className="mt-3 w-full rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white">Save</button></form>:null}
