 ...
 return <AppShell><div className="mx-auto max-w-6xl px-5 py-8 sm:px-8">
  ...
  <div className="mt-6 grid gap-4 md:grid-cols-[1fr_300px]">
   ...
   <div className="space-y-4">
    {canEdit&&collection.kind!=="favorites"?<form action={updateCollection.bind(null,id)} className="rounded-2xl border border-black/[0.06] bg-white p-5"><h2 className="text-sm font-semibold">{collection.kind==="smart"?"Edit smart collection":"Edit collection"}</h2><input name="name" defaultValue={collection.name} className="mt-3 h-10 w-full rounded-xl border border-slate-200 px-3 text-sm"/>{collection.kind==="smart"?<div className="mt-3 grid gap-2"><select name="rule_kind" className="h-10 rounded-xl border border-slate-200 px-2 text-sm"><option value="kind">Card kind</option><option value="marked">Marked</option><option value="suspended">Suspended</option><option value="tag">Tag</option></select><input name="rule_value" defaultValue={String(collection.rule?.tag||collection.rule?.kind||"")} placeholder="kind or tag name" className="h-10 rounded-xl border border-slate-200 px-3 text-sm"/></div>:null}<button className="mt-3 w-full rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white">Save</button></form>:null}
