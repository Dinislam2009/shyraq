...
export default async function termsPage(){
 ...
 return <AppShell><article className="mx-auto max-w-3xl px-5 py-10 sm:px-8">
  ...
  <h1 className="mt-2 text-3xl font-semibold tracking-tight">{t("termsTitle")}</h1>
  <p className="mt-3 text-sm leading-6 text-slate-500">{t("termsIntro")}</p>
  ...
  <div className="prose prose-slate mt-8 max-w-none dark:prose-invert">
   <h2>{t("termsUseTitle")}</h2><p>{t("termsUseText")}</p>
   <h2>{t("termsPublicTitle")}</h2><p>{t("termsPublicText")}</p>
   <h2>{t("termsAccountTitle")}</h2><p>{t("termsAccountText")}</p>
  </div>
