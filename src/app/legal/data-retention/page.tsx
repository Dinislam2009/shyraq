...
export default async function data_retentionPage(){
 ...
 return <AppShell><article className="mx-auto max-w-3xl px-5 py-10 sm:px-8">
  ...
  <h1 className="mt-2 text-3xl font-semibold tracking-tight">{t("retentionTitle")}</h1>
  <p className="mt-3 text-sm leading-6 text-slate-500">{t("retentionIntro")}</p>
  ...
  <div className="prose prose-slate mt-8 max-w-none dark:prose-invert">
   <h2>{t("retentionActiveTitle")}</h2><p>{t("retentionActiveText")}</p>
   <h2>{t("retentionDeletionTitle")}</h2><p>{t("retentionDeletionText")}</p>
   <h2>{t("retentionBackupsTitle")}</h2><p>{t("retentionBackupsText")}</p>
  </div>
