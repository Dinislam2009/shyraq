...
export default async function EditCardPage({params}:{params:Promise<{id:string;cardId:string}>}){
 ...
 const initial={
  ...
  markers:Array.isArray(card.content?.markers)?card.content.markers:[],
  status:typeof card.content?.status==="string"?card.content.status:"",
  fields:card.content?.fields&&typeof card.content.fields==="object"&&!Array.isArray(card.content.fields)?card.content.fields:{},
  reviewPreferences:card.content?.reviewPreferences&&typeof card.content.reviewPreferences==="object"&&!Array.isArray(card.content.reviewPreferences)?card.content.reviewPreferences:{},
