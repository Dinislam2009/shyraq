...
export default async function WorkspacePage(){
 ...
 const {data:invites}=await supabase.from("workspace_invitations").select("email,role,expires_at,accepted_at,created_at").eq("workspace_id",workspace.id).order("created_at",{ascending:false}).limit(20);
