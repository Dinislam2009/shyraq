...
export async function getReviewCard(){
 ...
 const {data:due}=await supabase.from("review_states")
   ...
   .limit(1);
 if(due?.[0]){
   const {data:card}=await supabase.from("cards").select("id,deck_id,kind,content").eq("id",due[0].card_id).maybeSingle();
   if(card)return {card,stateData:due[0].state_data,isNew:false};
 }
...
export async function getReviewStats(){
 const supabase=await createClient();
 const {data:{user}}=await supabase.auth.getUser();
 if(!user)return {reviews:0,accuracy:null,studyMinutes:0};
 const {count:reviews}=await supabase.from("review_events").select("*",{count:"exact",head:true}).eq("user_id",user.id);
 const {data:events}=await supabase.from("review_events").select("rating,elapsed_ms").eq("user_id",user.id);
 const list=events??[];
 const good=list.filter((e:any)=>e.rating!=="again").length;
 const studyMinutes=Math.round(list.reduce((sum:number,e:any)=>sum+Number(e.elapsed_ms||0),0)/60000);
 return {reviews:reviews??0,accuracy:list.length?Math.round(good/list.length*100):null,studyMinutes};
}
