...
export function OfflineSyncPanel({userId,devices}:{userId:string;devices:Device[]}){
 ...
 const [overview,setOverview]=useState({reviews:0,reviewCache:0,decks:0,cards:0,cardTemplates:0,tags:0,collections:0,collectionCards:0,reviewStates:0,mutations:0,mediaFiles:0,mediaBytes:0});
 ...
 return <div className="mt-8 space-y-5">
  ...
  <div className="rounded-2xl border border-black/[0.06] bg-white p-5">
   ...
   <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-8"><Info label="Reviews" value={String(overview.reviews)}/><Info label="Review states" value={String(overview.reviewStates)}/><Info label="Cards" value={String(overview.cards)}/><Info label="Templates" value={String(overview.cardTemplates)}/><Info label="Tags" value={String(overview.tags)}/><Info label="Collections" value={String(overview.collections)}/><Info label="Decks" value={String(overview.decks)}/><Info label="Mutations" value={String(overview.mutations)}/><Info label="Media" value={String(overview.mediaFiles)}/></div>
   <StorageMeter/>
  </div>
  <div className="rounded-2xl border border-black/[0.06] bg-white p-5">
