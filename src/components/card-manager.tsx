...
export function CardManager({ deckId, cards, favoriteIds }: { deckId: string; cards: CardRow[]; favoriteIds: string[] }) {
            ...
            <div key={card.id} className={"p-6 " + (index ? "border-t border-black/[0.05]" : "")}>
              <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                <div className="flex items-start gap-3">
                  <input type="checkbox" checked={selected.includes(card.id)} onChange={() => toggle(card.id)} className="mt-1 h-4 w-4 rounded border-slate-300" />
                  <div>
