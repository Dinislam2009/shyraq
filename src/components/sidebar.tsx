...
const labelKeys = {
  ...
  "Public decks":"publicDecks","Collections":"collections","Import":"import","Export":"export",
  "Profile":"profile","Review settings":"reviewSettings","Sync":"sync","Moderation":"moderation",
  "Workspace":"workspaceSettings","Settings":"settings",
} as const;
    ...
    <aside className="hidden w-64 shrink-0 border-r border-black/[0.06] bg-white lg:flex lg:flex-col">
      ...
      <nav className="flex-1 px-3 py-5">
        ...
        <div className="space-y-1">
          {mainNavigation.map((item) => {
            ...
            return <Link key={item.href} href={item.href} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${active ? "bg-slate-100 text-slate-950" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}><Icon size={18} />{t(labelKeys[item.label as keyof typeof labelKeys])}</Link>;
          })}
        </div>
      ...
      <div className="border-t border-black/[0.06] p-4">
        <div className="rounded-2xl bg-slate-50 p-4">
          <div className="mb-2 text-xs font-semibold text-slate-900">{t("dailyReview")}</div>
          <p className="text-xs leading-5 text-slate-500">{t("dailyReviewText")}</p>
        </div>
      </div>
    </aside>
  );
