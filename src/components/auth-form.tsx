...
export function AuthForm({ mode }: { mode: "login" | "signup" }) {
  ...
  return <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-sm ring-1 ring-black/5">
    ...
    <div className="mt-5 grid gap-2 sm:grid-cols-2"><a href="/api/auth/oauth/google?next=%2Fdashboard" className="flex h-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-sm font-semibold">Continue with Google</a><a href="/api/auth/oauth/apple?next=%2Fdashboard" className="flex h-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-sm font-semibold">Continue with Apple</a></div><div className="my-5 flex items-center gap-3 text-[11px] text-slate-400"><span className="h-px flex-1 bg-slate-100"/><span>or</span><span className="h-px flex-1 bg-slate-100"/></div><form onSubmit={submit} className="space-y-4">
