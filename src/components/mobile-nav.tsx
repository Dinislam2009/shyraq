...
export function MobileNav(){
 ...
 return <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-black/[0.06] bg-white/95 px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur dark:border-white/10 dark:bg-slate-950/95 lg:hidden">
  ...
  <div className="mx-auto grid max-w-lg grid-cols-5 gap-1">
   ...
   <button type="button" aria-expanded={open} onClick={()=>setOpen(value=>!value)} className={"flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl text-[11px] font-medium "+(open?"bg-slate-100 text-slate-950 dark:bg-slate-800 dark:text-white":"text-slate-500")}><SettingsIcon size={18}/>{t("more")}</button>
  </div>
 </nav>;
}
