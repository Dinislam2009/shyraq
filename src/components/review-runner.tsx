...
type QueueItem={card:{id:string;content:CardContent;kind:string;is_suspended?:boolean;template_id?:string;card_templates?:{id:string;name:string;front_template:string;back_template:string;css?:string|null}|{id:string;name:string;front_template:string;back_template:string;css?:string|null}[]};stateData:any;isNew:boolean};
...
export function ReviewRunner({userId,queue,preferences}:{userId:string;queue:QueueItem[];preferences:Preferences}){
 ...
 const {front,back,templateCss,usesAnkiTemplate}=useMemo(()=>{
   ...
   if(template){
    ...
    const specialFields:Record<string,string>={Tags:tags,Type:String(card.content?.anki?.modelName||card.kind||""),Deck:deckName,Subdeck:subdeck,Card:template.name,CardFlag:String(card.content?.anki?.flags||0),CardSuspended:card.is_suspended?"1":"0"};
