...
export function ReviewRunner({userId,card,stateData,preferences,isNew}:{userId:string;card:{id:string;content:CardContent;kind:string};stateData:any;preferences:Preferences;isNew:boolean}){
 const [revealed,setRevealed]=useState(false);const [busy,setBusy]=useState(false);const [saved,setSaved]=useState(false);const router=useRouter();const shellRef=useRef<HTMLDivElement>(null);
 ...
 useEffect(()=>{
  const onKeyDown=(event:KeyboardEvent)=>{
   ...
   if(event.code==="Space"){event.preventDefault();if(!revealed)setRevealed(true);}
 ...
 const rawFront=card.content?.front||"";const rawBack=card.content?.back||"";
 ...
 return <div ref={shellRef} tabIndex={0} className="outline-none mx-auto flex min-h-[calc(100vh-4.5rem)] max-w-4xl flex-col px-5 py-8 sm:px-8"><div className="mb-6 flex items-center justify-between"><div><p className="text-xs font-medium uppercase tracking-[0.14em] text-slate-400">{card.kind}</p><p className="mt-1 text-sm text-slate-500">{isNew?"New card":"Scheduled review"}</p></div></div><div className="flex flex-1 items-center"><div className="w-full rounded-3xl border border-black/[0.06] bg-white p-8 text-center shadow-sm sm:p-12"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Front</p><h1 className="mx-auto mt-6 max-w-2xl whitespace-pre-wrap text-3xl font-semibold">{front}</h1>{card.kind==="image"&&card.content.imageUrl&&<img src={card.content.imageUrl} alt="" className="mx-auto mt-8 max-h-72 rounded-2xl object-contain"/>}{card.content.mediaUrl&&card.content.mediaType?.startsWith("image/")&&<img src={card.content.mediaUrl} alt="" className="mx-auto mt-8 max-h-72 rounded-2xl object-contain"/>}{card.content.mediaUrl&&card.content.mediaType?.startsWith("audio/")&&<audio controls src={card.content.mediaUrl} className="mx-auto mt-8 w-full max-w-xl"/>}{card.content.mediaUrl&&card.content.mediaType?.startsWith("video/")&&<video controls src={card.content.mediaUrl} className="mx-auto mt-8 max-h-72 w-full rounded-2xl"/>}{card.content.mediaItems?.map((item,index)=><div key={item.path+index} className="mt-8">{item.mime_type.startsWith("image/")&&item.url&&<img src={item.url} alt="" className="mx-auto max-h-72 rounded-2xl object-contain"/>}{item.mime_type.startsWith("audio/")&&item.url&&<audio controls src={item.url} className="mx-auto w-full max-w-xl"/>}{item.mime_type.startsWith("video/")&&item.url&&<video controls src={item.url} className="mx-auto max-h-72 w-full rounded-2xl"/>}</div>)}{card.kind==="multiple_choice"&&<div className="mx-auto mt-8 max-w-xl space-y-2 text-left">{(card.content.options??[]).map((option,index)=><div key={option+index} className="rounded-xl border border-slate-200 px-4 py-3 text-sm">{option}</div>)}</div>}{revealed?<div className="mt-10 border-t border-slate-100 pt-8"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Back</p>{card.kind==="multiple_choice"&&<p className="mt-5 text-sm font-semibold text-slate-900">Correct option: {(card.content.options??[])[card.content.answer??0]||"—"}</p>}<p className="mx-auto mt-5 max-w-2xl whitespace-pre-wrap text-lg leading-8 text-slate-600">{back}</p></div>:<button onClick={()=>setRevealed(true)} className="mt-12 rounded-xl border border-slate-200 px-6 py-3 text-sm font-semibold hover:bg-slate-50">Show answer <span className="ml-2 text-xs text-slate-400">Space</span></button>}</div></div>{revealed&&<div className="mt-5 grid grid-cols-4 gap-2">{(["again","hard","good","easy"] as const).map(r=><button key={r} disabled={busy} onClick={()=>answer(r)} className="rounded-xl border border-slate-200 bg-white py-3 text-sm font-semibold capitalize hover:bg-slate-50 disabled:opacity-50">{r}</button>)}</div>}</div>;
}