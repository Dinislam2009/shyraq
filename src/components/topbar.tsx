...
export async function Topbar(){
 ...
 return <header className="flex h-18 items-center justify-between border-b border-black/[0.06] bg-white/90 px-5 backdrop-blur sm:px-8">
  ...
  <div className="ml-auto flex items-center gap-3">
   <LanguageSwitcher/>
   <Link href="/history" className="hidden rounded-lg px-2.5 py-2 text-xs font-semibold text-slate-500 hover:bg-slate-100 hover:text-slate-900 sm:inline-flex">History</Link>
   ...
   <details className="relative"><summary className="flex h-9 w-9 cursor-pointer list-none items-center justify-center rounded-full bg-slate-200 text-xs font-semibold text-slate-700">{name.slice(0,1).toUpperCase()}</summary><div className="absolute right-0 z-20 mt-2 w-52 rounded-xl border border-slate-200 bg-white p-1 shadow-lg"><Link href="/settings/devices" className="block rounded-lg px-3 py-2 text-sm hover:bg-slate-50">Review devices</Link><Link href="/settings/profile" className="block rounded-lg px-3 py-2 text-sm hover:bg-slate-50">Profile</Link><form action={signOut}><button className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-50">Sign out</button></form></div></details>
  </div>
 </header>;
}