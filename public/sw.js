...
self.addEventListener("sync",event=>{if(event.tag==="shyraq-sync"){event.waitUntil(self.clients.matchAll({type:"window",includeUncontrolled:true}).then(clients=>clients.forEach(client=>client.postMessage({type:"shyraq-sync-request"}))).catch(()=>{}));}});
