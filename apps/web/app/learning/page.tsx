...
function LearningPageContent() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getDueFlashcards(selected.id, session.access_token),
      getFlashcardProgress(selected.id, session.access_token),
    ]).then(([nextCards, nextDueCards, nextProgress]) => {
      setCards(nextCards);
      setDueCards(nextDueCards);
      setProgress(nextProgress);
    }).catch((e) => setError(e instanceof Error ? e.message : "Failed to load learning data"));
  }, [session, selected]);
  ...
  async function addCard() {
    ...
    try {
      ...
      setDueCards((items) => [...items, card]);
      setProgress((value) => value ? { ...value, cardCount: value.cardCount + 1, dueCount: value.dueCount + 1, completionPercent: 0 } : value);
      setFront(""); setBack("");
  ...
  async function removeCard(cardId: string) {
    if (!selected) return;
    try {
      await deleteFlashcard(selected.id, cardId, session!.access_token);
      setCards((items) => items.filter((card) => card.id !== cardId));
      setDueCards((items) => items.filter((card) => card.id !== cardId));
  ...
  async function archiveDeck(deckId: string) {
    try {
      await archiveFlashcardDeck(deckId, session!.access_token);
      setDecks((items) => items.filter((deck) => deck.id !== deckId));
      if (selected?.id === deckId) { setSelected(null); setCards([]); setDueCards([]); setProgress(null); }
  ...
  async function submitReview(rating: ReviewRating) {
    ...
    try {
      await reviewFlashcard(selected.id, current.id, rating, session!.access_token);
      const [nextDueCards, nextProgress] = await Promise.all([
        getDueFlashcards(selected.id, session!.access_token),
        getFlashcardProgress(selected.id, session!.access_token),
      ]);
      setDueCards(nextDueCards);
  ...
  return (
    <main className="app-shell">
      ...
      <section className="content-panel">
        <header className="topbar"><div><p className="eyebrow">Phase 9</p><h1>Learning</h1></div></header>
        ...
        <div className="board-grid">
          ...
          <section className="card-panel">
            ...
            {selected && <>
              {progress && <div className="progress-summary" aria-label="Learning progress"><strong>{progress.dueCount} due</strong><span>{progress.reviewedCardCount}/{progress.cardCount} cards · {progress.correctCount}/{progress.reviewCount} correct</span></div>}
              <div className="review-card" onClick={() => setRevealed((value) => !value)} role="button" tabIndex={0}>
                {current ? <><strong>{revealed ? current.back : current.front}</strong><span>{revealed ? "Answer" : "Question"}</span></> : <span>No cards due right now</span>}
              </div>
              {current && <>
                <div className="inline-form"><button onClick={() => { setIndex((value) => (value + dueCards.length - 1) % dueCards.length); setRevealed(false); }}>Previous</button><span>{index + 1} / {dueCards.length}</span><button onClick={() => { setIndex((value) => (value + 1) % dueCards.length); setRevealed(false); }}>Next</button></div>
                {revealed && <div className="inline-form"><button onClick={() => submitReview("AGAIN")}>Again</button><button onClick={() => submitReview("HARD")}>Hard</button><button onClick={() => submitReview("GOOD")}>Good</button><button onClick={() => submitReview("EASY")}>Easy</button></div>}
              </>}
